...
#[contracttype]
pub enum DataKey {
    /// Offer keyed by (maker, offer_id).
    Offer(Address, u64),
    /// Per-maker offer counter.
...
/// `output_amount` they are willing to pay; execution only proceeds if
...
pub struct Offer {
    pub id: u64,
    pub maker: Address,
    /// Token the maker is selling (deposits into contract on offer creation).
    pub token_in: Address,
    /// Amount of `token_in` being offered.
    pub amount_in: i128,
    /// Token the maker wants to receive.
    pub token_out: Address,
    /// Minimum acceptable amount of `token_out` (slippage guard).
    pub min_output: i128,
    pub filled: bool,
    pub cancelled: bool,
}
